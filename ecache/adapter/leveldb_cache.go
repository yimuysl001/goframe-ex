...
func (a *AdapterLeveldb) Clear(ctx context.Context) error {
	...
	for iter.Next() {
		...
		if err != nil {
			...
			g.Log().Error(ctx, fmt.Sprintf("删除数据失败：%v", err))
			continue
		}
		g.Log().Error(ctx, fmt.Sprintf("已删除数据：%s\n", key))
...
// NewLevelDbAdapterFile creates and returns a new memory cache object.
func NewLevelDbAdapterFile(dir string) gcache.Adapter {
