...
func NewServer(name string, namespace string) IServer {
	g.Log().Debug(gctx.GetInitCtx(), "webservice:", namespace+"?wsdl")
...
func (s *server) RegisterMethod(m Method) error {
	...
	if _, ok := s.methods[tname]; ok {
		g.Log().Error(gctx.GetInitCtx(), "方法重复注册:"+tname)
	...
	if erro != nil {
		g.Log().Error(gctx.GetInitCtx(), erro)
		return erro
	}
	...
	g.Log().Debug(gctx.GetInitCtx(), "已注册服务：", tname)
	s.regWsdl(tname)
	s.methods[tname] = m
	return nil
}
